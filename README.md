        ...
        stage('Aws Docker Deploy') {
            ...
            steps {
                script {
                    ...
                    try {
                        echo "Triggering 'aws_docker_deploy_job_numerical' with parameters:"

                        def dockerDeployJob = build(
                            job: 'aws_docker_deploy_job_numerical', // Ensure this is the exact job name
                            parameters: [
                                string(name: 'NGINX_CONF', value: NGINX_CONF),
                                string(name: 'SERVER_NAME', value: SERVER_NAME),
                                string(name: 'DOCKER_PORT', value: DOCKER_PORT),
                                string(name: 'AWS_PUBLIC_IP', value: AWS_IP),
                                string(name: 'PROJECT_NAME_WITH_DASH', value: PROJECT_NAME_WITH_DASH),
                                string(name: 'ENV_PROJECT_NAME', value: ENV_PROJECT_NAME),
                            ],
                            propagate: false, // Capture the result without failing the main pipeline immediately
                            wait: true
                        )

                        // Log the result of the job
                        echo "Triggered job 'aws_docker_deploy_job_numerical' completed with result: ${dockerDeployJob.result}"
                        ...
                        echo "An error occurred while trying to trigger 'aws_docker_deploy_job_numerical'."
                        echo "Exception message: ${e.getMessage()}"
                        e.printStackTrace()  // Prints the full stack trace for more insight
                        currentBuild.result = 'FAILURE'
                        error("Stopping main pipeline due to an error when calling AWS Docker deployment job.")
                    }
                }
            }
        }
